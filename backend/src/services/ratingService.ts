...
export function calculateHauntedRating(
  ...
  const breakdown = generateFactorBreakdown(location, environmental, {
...
function generateFactorBreakdown(
  location: Location,
  environmental: EnvironmentalFactors,
  factors: { locationScore: number; weatherScore: number; timeScore: number; seasonScore: number }
): FactorBreakdown[] {
  ...
  breakdown.push({
    ...
    contribution: Math.round(factors.locationScore * FACTOR_WEIGHTS.LOCATION),
  ...
  breakdown.push({
    ...
    contribution: Math.round(factors.weatherScore * FACTOR_WEIGHTS.WEATHER),
  ...
  breakdown.push({
    ...
    contribution: Math.round(factors.timeScore * FACTOR_WEIGHTS.TIME),
  ...
  breakdown.push({
    ...
    contribution: Math.round(factors.seasonScore * FACTOR_WEIGHTS.SEASON),
...
function getLocationDescription(locationType: LocationType): string {
  const descriptions: Record<LocationType, string> = {
    [LocationType.CASTLE]: 'Ancient castles are steeped in history and countless tales of tragedy, making them prime locations for supernatural activity.',
    [LocationType.GRAVEYARD]: 'Graveyards and cemeteries are traditional gathering places for spirits, with strong connections to the afterlife.',
    [LocationType.ABANDONED_BUILDING]: 'Abandoned structures often harbor residual energy from past inhabitants and traumatic events.',
    [LocationType.FORT]: 'Military fortifications carry the weight of battles fought and lives lost, creating powerful spiritual imprints.',
    [LocationType.REGULAR]: 'Regular locations have minimal supernatural associations, though spirits can manifest anywhere.'
  };
