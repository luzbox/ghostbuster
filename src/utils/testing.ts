...
export class TestSuite {
  ...
  private async testErrorHandling(): Promise<void> {
    try {
      const { parseError } = await import('./errorHandling');
      
      const testError = new Error('Test error with TEST_ERROR code');
      
      const parsed = parseError(testError);
      
      this.assert(
        'Error parsing returns correct structure',
        parsed.message.includes('TEST_ERROR')
      );
      ...
      this.assert(
        ...
        parsedNetwork.message.includes('fetch failed')
      );
...
export const runDevelopmentTests = async (): Promise<void> => {
  if (import.meta.env.DEV) {
    ...
    await benchmark.benchmark('Rating Calculation', async () => {
      const { calculateHauntedRating } = await import('./ratingEngine');
      return calculateHauntedRating(mockLocation, mockEnvironmentalFactors);
    });
