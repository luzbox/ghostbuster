...
import { analyzeLocation, calculateHauntedRating } from './services/api';
import { startAutoRefresh, stopAutoRefreshByLocation, getCachedRating } from './services/realTimeService';
import { PerformanceMonitor, measureAsync, registerServiceWorker, getOptimalSettings } from './utils/performance';
...
function App() {
  ...
  useEffect(() => {
    if (currentLocation) {
      const sessionKey = startAutoRefresh(currentLocation, {
        ...
        onError: (errorMessage) => {
          ...
          const cachedRating = getCachedRating(currentLocation);
          if (cachedRating) {
      ...
      // Cleanup function
      return () => {
        stopAutoRefreshByLocation(currentLocation);
      };
    }
      ...
      await measureAsync(async () => {
        ...
        try {
          ...
          try {
            ...
            cachedRating = getCachedRating(location);
