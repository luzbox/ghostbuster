...
import { calculateHauntedRating, getCurrentWeather, getEnvironmentalFactors } from './api';
...
const refreshSessionData = async (sessionKey: string): Promise<void> => {
  ...
  try {
    ...
    cacheData(ratingCache, cacheKey, rating);
    
    // Notify callback
    if (session.onRatingUpdate) {
      session.onRatingUpdate(rating);
    }
    
    console.log(`[REAL-TIME] Successfully refreshed rating for ${session.location.name}: ${rating.overallScore}/100`);
