...
const getCachedWeather = (lat: number, lon: number): EnvironmentalFactors | null => {
  ...
  if (cached && isCacheValid(cached)) {
    return cached.data as unknown as EnvironmentalFactors;
  }
...
const transformWeatherData = (weatherData: WeatherApiResponse): EnvironmentalFactors => {
  ...
  return {
    ...
    time: {
      hour,
      isNighttime: isNighttime(currentTime, weatherData.sys?.sunrise || 0, weatherData.sys?.sunset || 0),
      timezone: `UTC${timezoneOffsetSeconds >= 0 ? '+' : ''}${timezoneOffsetSeconds / 3600}`
    },
    season: calculateSeason(currentDate, weatherData.coord?.lat || 0)
  };
};
...
export const getCurrentEnvironmentalFactors = async (
  ...
  try {
    ...
    if (staleCache) {
      ...
      return staleCache.data as unknown as EnvironmentalFactors;
    }
